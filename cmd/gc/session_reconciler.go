...
func finalizeDrainAckStoppedSessionWithAssignedWork(
	...
	if closeIfUnassigned && !hasAssignedWork {
		if closeSessionBeadKnownUnassigned(store, *session, "drained", clk.Now().UTC(), stderr) {
...
func reconcileSessionBeadsTracedWithNamedDemand(
	...
	for i := range ordered {
		...
		if reconcileDrainAckStopPending(cityPath, cfg, sp, store, rigStores, session, tp, desired, stoppedAssignedWorkDecision, dops, dt, clk, rec, stderr) {
			continue
		}
				...
				if cfgAgent != nil {
					...
					if storedHash != currentHash {
						...
						if !restartedInPlace {
							...
							if hasAssignedWork {
								if it != nil && it.checkIdle(name, sp, clk.Now()) {
